...
- add `CmdChangeSidebarPushMode` (`Change Sidebar Push Mode`) command to open dialog for choosing whether opening the left sidebar pushes the document content aside: `always` (legacy), `auto` (push only when needed), or `never` (sidebar overlaps content)
